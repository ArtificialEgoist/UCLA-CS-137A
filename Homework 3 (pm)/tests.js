...
tests(
  ...
  },
  {
    name: 'many example #1',
    code: 'match([1,2,3],\n' +
          '   [many(_)], function(x) { return JSON.stringify(arguments); }\n' +
          ')',
    expected: "{\"0\":[1,2,3]}"
  },
  {
    name: 'many example #2',
    code: 'match([1,2,3],\n' +
          '   [many(when(isOne)),many(_)], function(x,y) { return JSON.stringify(arguments); }\n' +
          ')',
    expected: "{\"0\":[1],\"1\":[2,3]}"
  },
  {
  ...
  },
  {
    name: 'many example #4',
    code: 'match([1,2,3],\n' +
          '   [many(when(isOne)),2,many(_)], function(x,y) { return JSON.stringify(arguments); }\n' +
          ')',
    expected: "{\"0\":[1],\"1\":[3]}"
  },
  {
    name: 'many example #5',
    code: 'match([[1,2],[3,4]],\n' +
          '   [many([many(_)])], function(x) { return JSON.stringify(arguments); }\n' +
          ')',
    expected: "{\"0\":[[1,2],[3,4]]}"
  },
  /*
  {
    name: 'nested many pairs',
    code: 'match([[1, 2], [3, 4], [5, 6]],\n' +
          '  [many([many(_)])], function(pts) { return JSON.stringify(pts); }\n' +
          ')',
    expected: "[1,2,3,4,5,6]"
  },
  */
  {
    name: 'match failure after many',
    code: 'match([1, 2, 3, "and", 4],\n' +
          '  [many(_), "and", _], function(xs, s) { console.log(xs); console.log(s); return 1; }\n' +
          ')',
    shouldThrow: true
  },
  {
    name: 'pattern match after many',
    code: 'match([1, 2, 3, "and", 4],\n' +
          '  [many(when(isNumber)), "and", _], function(xs, x) { return JSON.stringify(xs); }\n' +
          ')',
    expected: "[1,2,3]"
  },
  {
